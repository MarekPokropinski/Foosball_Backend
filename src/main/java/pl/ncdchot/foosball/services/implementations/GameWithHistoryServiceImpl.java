...
public abstract class GameWithHistoryServiceImpl extends GameServiceImpl implements GameWithHistoryService {
	...
	@Override
	public long startGame(long[] redTeamUsers, long[] blueTeamUsers, Rules rules, GameType gameType)
	...
	private Game createGame(long[] redTeamUsers, long[] blueTeamUsers, Rules rules, GameType gameType)
			throws UserNotExistException {
		Team redTeam = createTeam(redTeamUsers);
		Team blueTeam = createTeam(blueTeamUsers);

		Rules savedRules = rulesService.getRules(rules);
		return getNewGame(gameType, savedRules, redTeam, blueTeam);
	}

	private Team createTeam(long[] users) throws UserNotExistException {
		Team team;
		User firstUser = getUser(users[FIRST_PLAYER_INDEX]);
		if (isGameSolo(users)) {
			team = teamService.getTeamByUsers(firstUser);
		} else {
			User secondUser = getUser(users[SECOND_PLAYER_INDEX]);
	...
	public GameInfo getGameInfo(long gameId) throws GameNotFoundException {
		...
		Game game = gameService.getGame(gameId);
		if (game.getType().equals(GameType.FREE)) {
			return base;
		}
